use bincode::Options;
use vpp_api_gen::vpe::CliInband;
use vpp_api_gen::vpe::CliInbandReply;
...
fn main() {
    ...
    let create_host_interface: CliInbandReply = send_recv_msg(
        ...
        &CliInband::builder().client_index(t.get_client_index()).context(0).cmd("create host-interface name vpp1out".try_into().unwrap()).build().unwrap(), 
        &mut *t, 
