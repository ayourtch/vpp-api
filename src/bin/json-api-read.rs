...
fn generate_code(opts: &Opts, api_files: &LinkedHashMap<String, VppJsApiFile>) {
    ...
    acc.push_str("#![allow(non_camel_case_types)]\n");
    acc.push_str("use serde::{de::DeserializeOwned, Deserialize, Serialize};\n");
    acc.push_str("use vpp_api_encoding::typ::*;\n");
    acc.push_str("use vpp_api_transport::*;\n");
    ...
    for (name, f) in api_files {
        ...
        for m in &f.unions {
            ...
            acc.push_str(&format!("#[derive(Copy, Clone)]\n"));
        ...
        for (mname, m) in &f.aliases {
            ...
            if type_needs_copy_trait.get(&camel_case_name).is_some() {
                acc.push_str(&format!("#[derive(Copy)]\n"));
            }
